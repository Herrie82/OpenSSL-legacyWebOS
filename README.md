...
| `palm-install` / WOSQI installs fail with `FAILED_IPKG_INSTALL` (ipkg child exit 127) under LunaCE + luna-tls13 | Same leaked-env mechanism, but in LunaSysMgr's own App-Manager install path, which the wrapper can't intercept. Install via **Preware** (its ipkgservice doesn't run under LunaSysMgr's env) or over novacom: `ipkg -o /media/cryptofs/apps install <ipk>` then run the package's `pmPostInstall.script` with `IPKG_OFFLINE_ROOT=/media/cryptofs/apps`. |
