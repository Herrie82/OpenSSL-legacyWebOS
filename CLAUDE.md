...
2. `luna-tls13` — patches the `LunaSysMgr` upstart launcher → app WebKit (Mojo/Enyo XHR) on TLS 1.3, **+ a `media-pipeline` env-scrub wrapper (v1.1.1) so HTML5 *and* local media play reliably, + a `setcpushares-pdk` env-scrub wrapper (v1.1.2) so PDK apps launch, + a `setcpushares-task` env-scrub wrapper (v1.1.3) so App-Manager installs/removes (Preware/WOSQI) don't wedge** (see Key facts). **Needs #1; reboot after.**
